 ...
 * @return void *	Pointer to destination
 */
static inline void *rpmi_env_mem_write(char *dest, const void *buf, rpmi_size_t n)
{

		cpu_physical_memory_write((hwaddr)dest, buf, n);
		return (void *)dest;
}
 ...
 *
 * @param[in] src 	source hw address
 * @param[in] buf 	buffer pinter
 * @param[in] n 	number of bytes to write
 * @return void *	Pointer to destination
 */
static inline void *rpmi_env_mem_read(const void *src, char* buf, rpmi_size_t n)
{

		cpu_physical_memory_read((hwaddr)src, buf, n);
		return (void *)src;
}
...
static inline void *rpmi_env_memset(void *dest, int c, rpmi_size_t n)
{
	...
	while (n > 0) {
		n--;
		rpmi_env_mem_write(temp++, (void*)&c, 1);
	}
 ...
 */
static inline void *rpmi_env_zalloc(rpmi_size_t size)
{
 ...
 */
static inline void rpmi_env_free(void *ptr)
{
		free(ptr);
}
 ...
 */
static inline void rpmi_env_writel(rpmi_uint64_t addr, rpmi_uint32_t val)
{
 ...
 * @brief Console functions for logging purpose to be implemented by the
 * platform firmware.
 * @{
 */	
