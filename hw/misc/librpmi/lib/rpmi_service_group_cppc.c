 ...
 * A register even if not implemented is still a valid
 * register.
 */
...
/**
 * Get the hart fast channel offset
 */
...
/**
 * Get the desired_perf value from fastchannel for a hart
 */
static inline rpmi_uint32_t
__rpmi_cppc_get_desired_perf(struct rpmi_cppc_group *cppcgrp,
                             rpmi_uint32_t hart_index)
{
    int rc;
    rpmi_uint32_t val, offset;

    offset = hart_index * sizeof(union rpmi_cppc_hart_fastchan);

    rc = rpmi_shmem_read(cppcgrp->fastchan->shmem, offset, &val, 4);
    if (rc)
        return 0;
                                            ...
                                            rpmi_uint64_t *reg_val)
{
    ...
    case RPMI_CPPC_REFERENCE_PERF:
        ...
        break;
    case RPMI_CPPC_DESIRED_PERF:
        val = __rpmi_cppc_get_desired_perf(cppcgrp, hart_index);
        break;
    case RPMI_CPPC_PERF_LIMITED:
        status = cppcgrp->ops->cppc_get_reg(cppcgrp->ops_priv,
			            ...
			            rpmi_uint8_t *response_data)
{
    ...
    rpmi_uint32_t hart_id, cppc_reg_id, hart_index, reg_len = 0;
			            ...
			            rpmi_uint8_t *response_data)
{
    ...
    *response_datalen = 3 * sizeof(*resp);
...
static enum rpmi_error rpmi_cppc_process_events(struct rpmi_service_group *group)
{
    ...
    for (hart_idx = 0; hart_idx < cppcgrp->hart_count; hart_idx++) {
        ...
        desired_perf = __rpmi_cppc_get_desired_perf(cppcgrp, hart_idx);
                          ...
                          struct rpmi_shmem *shmem_fastchan)
{
    ...
    /* size if not 0 and its power of 2 */
    if (!size && (size & (size - 1))) {
    ...
    if (rpmi_shmem_size(shmem_fastchan) &
                    (sizeof(union rpmi_cppc_hart_fastchan) - 1))
		return NULL;
