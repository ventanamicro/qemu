						    ...
						    rpmi_uint8_t *response_data)
{
	...
	ret = sysreset_type ? RPMI_SYSRST_ATTRIBUTES_FLAGS_SUPPORTED : 0;

	*response_datalen = 2 * sizeof(*resp);
	resp[0] = rpmi_to_xe32(trans->is_be, (rpmi_uint32_t)RPMI_SUCCESS);
	resp[1] = rpmi_to_xe32(trans->is_be, ret);
...
static struct rpmi_service rpmi_sysreset_services[RPMI_SYSRST_SRV_MAX] = {
	...
	[RPMI_SYSRST_SRV_GET_SYSTEM_RESET_ATTRIBUTES] = {
		.service_id = RPMI_SYSRST_SRV_GET_SYSTEM_RESET_ATTRIBUTES,
		.min_a2p_request_datalen = 4,
				   ...
				   void *ops_priv)
{
	...
	group->max_service_id = RPMI_SYSRST_SRV_MAX;
