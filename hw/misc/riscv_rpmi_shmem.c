...
{
    rpmi_env_mem_read((void *)addr, in, len);
    return RPMI_SUCCESS;
}
...
static enum rpmi_error shmem_env_memcpy_write(void *priv, rpmi_uint64_t addr,
                                              ...
                                              rpmi_uint32_t len)
{
    rpmi_env_mem_write((void *)addr, out, len);
    return RPMI_SUCCESS;
}
...
{
    rpmi_env_memset((void *)addr, ch, len);
    return RPMI_SUCCESS;
}
...
struct rpmi_shmem *rpmi_shmem_qemu_create(const char *name, rpmi_uint64_t base,
                                           rpmi_uint32_t size)
{
    return rpmi_shmem_create(name, base, size, &rpmi_shmem_simple_ops, NULL);
}
