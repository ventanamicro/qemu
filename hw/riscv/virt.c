 ...
 * full_virt_memmap is capable of holding both virt_memmap and VIRT_HIGH_PCIE_MMIO
 * entry.
 *
                                   ...
                                   bool is_32_bit, uint32_t *intc_phandles)
{
    ...
    for (cpu = s->soc[socket].num_harts - 1; cpu >= 0; cpu--) {
        ...
        if (s->soc[socket].harts[cpu].cfg.ext_icbom) {
            qemu_fdt_setprop_cell(mc->fdt, cpu_name, "riscv,cbom-block-size",
            s->soc[socket].harts[cpu].cfg.cbom_blocksize);
        }
                                          ...
                                          RISCVVirtState *vms)
{
    ...
    vms->bus = PCI_HOST_BRIDGE(dev)->bus;
    return dev;
}
